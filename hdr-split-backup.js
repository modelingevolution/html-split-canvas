class HDRSplitCanvas {
    constructor(canvas, grayscaleBar) {
        this.canvas = canvas;
    ...
    updateInfoBox(grayValue) {
        if (!this.infoBox) return;

        if (grayValue !== null) {
            const weight = this.weights[grayValue];
            const hex = grayValue.toString(16).padStart(2, '0').toUpperCase();
            const colorRgb = `rgb(${grayValue}, ${grayValue}, ${grayValue})`;

            this.infoBox.innerHTML = `
                <div style="display: flex; align-items: center; min-width: 140px;">
                    <span style="color: #888;">Gray Value:</span>&nbsp;<strong style="display: inline-block; width: 35px; text-align: right;">${grayValue}</strong>
                </div>
                <div style="display: flex; align-items: center; gap: 8px; min-width: 180px;">
                    <span style="color: #888;">Hex:</span>&nbsp;<strong style="display: inline-block; width: 70px;">#${hex}${hex}${hex}</strong>
                    <div style="width: 24px; height: 24px; background: ${colorRgb}; border: 1px solid #666; border-radius: 3px;"></div>
                </div>
                <div style="display: flex; align-items: center; min-width: 140px;">
                    <span style="color: #888;">Weight:</span>&nbsp;<strong style="display: inline-block; width: 50px; text-align: right;">${weight.toFixed(3)}</strong>
                </div>
            `;
        } else {
            this.infoBox.innerHTML = `
                <div style="display: flex; align-items: center; min-width: 140px;">
                    <span style="color: #888;">Gray Value:</span>&nbsp;<strong style="display: inline-block; width: 35px; text-align: right;">--</strong>
                </div>
                <div style="display: flex; align-items: center; gap: 8px; min-width: 180px;">
                    <span style="color: #888;">Hex:</span>&nbsp;<strong style="display: inline-block; width: 70px;">--</strong>
                    <div style="width: 24px; height: 24px; background: #333; border: 1px solid #666; border-radius: 3px;"></div>
                </div>
                <div style="display: flex; align-items: center; min-width: 140px;">
                    <span style="color: #888;">Weight:</span>&nbsp;<strong style="display: inline-block; width: 50px; text-align: right;">--</strong>
                </div>
            `;
        }
    }
...
window.addEventListener('DOMContentLoaded', () => {
    ...
    const hdrSplit = new HDRSplitCanvas(canvas, grayscaleBar);
